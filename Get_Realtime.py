...
def write_in_csvfiletitle(name):
    with open("Realtime_"+name+".csv", "w") as csvfile:
...
def write_in_csvfile(dict,name):
    with open("Realtime_"+name+".csv", "a+") as csvfile:
        writer = csv.writer(csvfile)
        for comment in dict:
...
def InitializeDB(TableName):     
    conn=MySQLdb.connect(host='localhost',user='root',passwd='W3087777',db='TESTDB',port=3306) 
    cur=conn.cursor()  
